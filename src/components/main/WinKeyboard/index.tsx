    ...
    const getItem = useCallback(() => {
        switch (item) {
            case Key.ArrowUp: {
                return (
                    <>
                        <div className={styles.btnArrowUp}>{t(`common.keyboard.win.keys.${item}`)}</div>
                        <div className={styles.btnArrowDown}>{t('common.keyboard.win.keys.ArrowDown')}</div>
                    </>
                );
            }
            case Key.Win: {
                return <WindowIcon fontSize="medium" />;
            }
            default: {
                return <div>{t(`common.keyboard.win.keys.${item}`)}</div>;
            }
        }
    }, [t, item]);
